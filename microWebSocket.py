        ...
        for x in range(10):
            try:
                ...
                try:
                    ...
                    _mws_thread_id = 0
                try:
                    start_new_thread('MWS_THREAD_%s' %
                                     _mws_thread_id, func, args)
                    return True
                except:
                    pass
        return False
                        ...
                        if self._msgType == self._msgTypeText:
                            if self.RecvTextCallback:
                                try:
                                    ...
                                    print(
                                        "MicroWebSocket : Error on recv text callback (%s)." % str(ex))
                        else:
                            if self.RecvBinaryCallback:
                                try:
                                    ...
                                    print(
                                        "MicroWebSocket : Error on recv binary callback (%s)." % str(ex))
                        self._msgType = None
                        self._msgLen = 0
                else:
                    return False
